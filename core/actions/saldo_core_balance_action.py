...
class SaldoCoreBalanceAction(ActionBase):
    def __init__(self, variables_base, contexto):
        super().__init__(variables_base, contexto, flow_name="saldo_core_balance", executor_type="desktop")
    ...
    def extraer_y_validar_billetera(self, paso):
        ...
        try:
            ...
            self.app_tools.presionar_combinacion_real("ctrl", "shift", "PgDn", repeticiones=5)
            ...
            billeteras_excluidas = {
                "MB_PLAN", "DATA_PLAN", "MIN_PLAN", "SMS_PLAN",
                "FUP_ILIMITADO", "FREE_ILIMITADO",
                "FUP_TETHERING", "FREE_TETHERING",
                "FREE_ TETHERING", "FUP_ TETHERING"
            }
            ...
            if resultado:
                ...
                self.logger.info(f"📥 Guardado en contexto '{campo_destino}': '{resultado_filtrado}'")
                return resultado_filtrado
            else:
                self.contexto[campo_destino] = None
                self.logger.info(f"⚠️ No se encontraron billeteras válidas, se guarda NULL en '{campo_destino}'")
                return None
