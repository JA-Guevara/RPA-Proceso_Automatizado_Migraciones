...
class PlanesSQLAdapter:
    ...
    def es_plan_valido(self, id_tipo_baja, id_tipo_lista, nombre_plan: str, tipo: str) -> bool:
        ...
        try:
            with SessionLocal() as db:
                sql = text(f"""
                    ...
                    WHERE id_tipo_baja = TRY_CONVERT(INT, :id_tipo_baja)
                      AND id_tipo_lista = TRY_CONVERT(INT, :id_tipo_lista)
                      AND (
            ...
            print(f"❌ Error validando plan '{nombre_plan}' "
                  f"(tipo_baja={id_tipo_baja}, tipo_lista={id_tipo_lista}, tipo={tipo}): {e}")
            return False
